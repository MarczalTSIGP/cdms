  ...
  context 'document_recipients:index' do
    ...
    context 'before import from csv' do
      should 'display buttons sucessufuly' do
        ...
        assert_selector '#main-content .card-header', text: I18n.t('views.document.recipients.nwdp', name: @document.title)
    ...
    context 'after import from csv' do
      ...
      context 'successfully' do
        ...
        should ' valid file - display @results ' do
          ...
          assert_selector '.card-body .alert', text: I18n.t('flash.actions.import.m', resource_name: I18n.t('activerecord.models.document_recipients.other'))

          assert_selector '#document_recipients_registered', text: I18n.t('views.document.recipients.import.document_recipients_registered')
          assert_selector '#invalids', text: I18n.t('views.document.recipients.import.invalids') # considere that exist at least one invalid member.
          
          find('#document_recipients_registered > div.card-header > div > a').click
          find('#invalids > div.card-header > div > a').click
          
          assert_selector '#document_recipients_registered > div.card-body > table > thead > tr > th', text: User.human_attribute_name(:name).upcase
          assert_selector '#document_recipients_registered > div.card-body > table > thead > tr > th', text: User.human_attribute_name(:cpf)
          assert_selector '#document_recipients_registered > div.card-body > table > thead > tr > th', text: User.human_attribute_name(:email).upcase
          
          @document.variables.each do |variable|
            assert_selector '#document_recipients_registered > div.card-body > table > thead > tr > th', text: "#{variable['name'].upcase} (#{variable['identifier'].upcase})" 
          end
