...
class Users::DocumentRecipientsControllerTest < ActionDispatch::IntegrationTest
  context 'authenticated' do
    ...
    should 'get from_csv' do
      ...
      assert_breadcrumbs({ link: users_root_path, text: I18n.t('views.breadcrumbs.home') }, # Pagina Inicial
                         { link: users_documents_path, text: Document.model_name.human(count: 2) }, # Certificados/Declarações
                         { link: users_document_path(@document), text: I18n.t('views.breadcrumbs.show', model: Document.model_name.human, id: @document.id) }, #Certificado/Declaração #3
                         { link: users_document_recipients_path(@document), text: I18n.t('views.document.recipients.plural') }, # Destinatários
                         { text: I18n.t('views.document.recipients.import.btn_csv') }) # Importar CSV
    end
    ...
    should 'post from_csv' do
      ...
      assert_difference "DocumentRecipient.count", 3 do
        post users_create_document_recipients_from_csv_path(@document.id), params: { csv: { file:  uploaded_file }, headers: { 'CONTENT_TYPE' => 'multipart/form-data' } }
      end
      
      assert_response :success
      assert_equal I18n.t('flash.actions.import.m', resource_name: I18n.t('activerecord.models.document_recipients.other')), flash[:success]
    ...
    should 'get download_csv' do
      ...
      @document.variables = [{'name' => 'codigo', 'identifier' => 'cod'}, {'name' => 'teste', 'identifier' => 'teste'}].to_json
      @document.save!
      ...
      normalized_str = ActiveSupport::Inflector.transliterate(@document.title + ' - Modelo de Importação de Destinatários')
      ...
      @document.variables.count.positive? ?  variables ="," : variables = ""
